...
export const DeleteParent = ({ id, showDeleteModal, setShowDeleteModal }) => {

    const [parents, setParents] = useState(initialState)
    const { err, success } = parents
    ...
    const handleSubmit = async e => {
        e.preventDefault()    
        try {
            const res = await axios.delete(`/admin/parents/delete/${id}`, {
                headers: {Authorization: adminToken}
            })

            setParents({...parents, err: '', success: res.data.msg})
            window.location.reload(false)
        } catch (err) {
            err.response.data.msg && 
            setParents({...parents, err: err.response.data.msg, success: ''})
        }
    }
