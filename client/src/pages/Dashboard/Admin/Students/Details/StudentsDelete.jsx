...
export const DeleteStudent = ({ id, showDeleteModal, setShowDeleteModal }) => {

    const [students, setStudents] = useState(initialState)
    const { err, success } = students
    ...
    const handleSubmit = async e => {
        e.preventDefault()    
        try {
            const res = await axios.delete(`/admin/students/delete/${id}`, {
                headers: {Authorization: adminToken}
            })

            setStudents({...students, err: '', success: res.data.msg})
            window.location.reload(false)
        } catch (err) {
            err.response.data.msg && 
            setStudents({...students, err: err.response.data.msg, success: ''})
        }
    }
