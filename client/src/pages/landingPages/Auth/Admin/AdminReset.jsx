...
const AdminReset = () => {
    ...
    const  { password, cf_password, err, success } = admin
    ...
    const handleResetPassword = async () => {
        if(isLength(password))
            return setAdmin({...admin, err: 'Password must be at least 6 characters.' , success: ''})
        
        if(!isStrong(password))
            return setAdmin({...admin, err: 'Password must contain one or more alphanumeric characters.' , success: ''})

        if(!isMatch(password, cf_password))
            return setAdmin({...admin, err: 'Password did not match.' , success: ''})
                                ...
                                <div className="mt-2 pt-8 p-2 md:p-8">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <div className="pt-2.5 pl-4">
                                            <Link to="/admin/login"><span className="text-blue-600">Login Now</span></Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
