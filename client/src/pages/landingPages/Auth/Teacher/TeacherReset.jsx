...
import './teacher.css'
import passwordReset from  '../../../../assets/img/mail.svg'
...
const TeacherReset = () => {
    ...
    const  { password, cf_password, err, success } = teacher
    ...
    const handleResetPassword = async () => {
        if(isLength(password))
            return setTeacher({...teacher, err: 'Password must be at least 6 characters.' , success: ''})
        
        if(!isStrong(password))
            return setTeacher({...teacher, err: 'Password must contain one or more alphanumeric characters.' , success: ''})

        if(!isMatch(password, cf_password))
            return setTeacher({...teacher, err: 'Password did not match.' , success: ''})

        try {
            const res = await axios.post('/teacher/reset', {password}, {
                headers: {Authorization: token}
            })
            return setTeacher({...teacher, err: '', success: res.data.msg})
                                ...
                                <div className="mt-2 pt-8 p-2 md:p-8">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <div className="pt-2.5 pl-4">
                                            <Link to="/teacher/login"><span className="text-blue-600">Login Now</span></Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
